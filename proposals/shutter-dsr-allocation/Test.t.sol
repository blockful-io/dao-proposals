...
contract Calldata is Test {
  ...
  function test_depositUSDCtoPot() external {
    ...
    approve = abi.encodeWithSelector(0x095ea7b3, DaiJoin, amount * decimalsDAI);
    (success, ) = DAI.call(approve);
    require(success, "DAI.approve failed");

    // Join DAI to Vat {SdrManager-join}
    bytes memory join = abi.encodeWithSelector(0x3b4da69f, Alice, amount * decimalsDAI);
    (success, ) = DaiJoin.call(join);
    require(success, "DaiJoin.join failed");
    ...
    // Dai balance {Vat-dai}
    bytes memory dai = abi.encodeWithSignature("dai(address)", Alice);
    (success, response) = Vat.staticcall(dai);
    require(success, "Vat.dai failed");
    uint256 daiBalance = abi.decode(response, (uint256));
    console2.log("daiBalance", daiBalance);

    // Join with DAI {Pot-join}
    uint256 RAY = 10 ** 27;
    join = abi.encodeWithSelector(0x049878f3, mul(amount, RAY) / chi);
